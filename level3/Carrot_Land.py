...
columns! But first, you need some land. And the only person who's selling
...
However, she can tell you the location of the three vertices, which lie on the
2-D plane and have integer coordinates.
...
coordinates on the 2-D plane. They must lie within the plot of land and not
on the boundaries. For example, if the vertices were (-1,-1), (1,0) and (0,
1), then you can plant only one carrot at (0,0).
...
The vertices list will contain exactly three elements, and each element will be a
list of two integers representing the x and y coordinates of a vertex. All
...
def answer(vertices):
    ...
    # Area
    area = 0.5 * abs((point_a[0] - point_c[0]) * (point_b[1] - point_a[1]) -
                   (point_a[0] - point_b[0]) * (point_c[1] - point_a[1]))

    # Boundary lattice points
    b_lattices = (gcd(abs(point_b[0] - point_a[0]), abs(point_b[1] - point_a[1]))
                  + gcd(abs(point_c[0] - point_b[0]), abs(point_c[1] - point_b[1]))
                  + gcd(abs(point_a[0] - point_c[0]), abs(point_a[1] - point_c[1])))
